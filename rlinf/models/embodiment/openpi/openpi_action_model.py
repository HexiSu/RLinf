...
from rlinf.models.embodiment.openpi.rtc_v2 import (
    RTCGuidanceContext,
)
from rlinf.models.embodiment.openpi.rtc_v2 import (
...
class OpenPi0Config(Pi0Config):
    ...
    max_delay: int = 0
    rtc_train_delay: int | None = None
    rtc_prefix_attention_horizon: int | None = None
    rtc_prefix_attention_end_mode: str = "scaled"
    rtc_prefix_attention_delay_multiplier: float = 2.0
    rtc_prefix_attention_offset: int = 0
    rtc_prefix_attention_schedule: str = "zeros"
    rtc_rate_of_inference: int = 30
...
class OpenPi0ForRLActionPrediction(PI0Pytorch, BasePolicy):
    ...
    def sft_forward(self, data, use_action_chunk_loss: bool = False, **kwargs):
        ...
        if self.config.rtc_enabled and self.config.max_delay > 0:
            from rlinf.models.embodiment.openpi.rtc_v2 import rtc_sft_loss

            loss = rtc_sft_loss(self, observation, actions)
