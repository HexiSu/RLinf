...
def validate_embodied_cfg(cfg):
    ...
    # The physical ROKAE gateway is intentionally single-instance and exposes
    # the 7D [six joints + gripper] contract.  Validate this at config time so
    ...
    for _env_name in ("train", "eval"):
        ...
        assert int(_env_cfg.get("total_num_envs", 1)) == 1, (
            ...
            f"got env.{_env_name}.total_num_envs={_env_cfg.get('total_num_envs')}"
        )
        assert int(model_cfg.get("action_dim", 0)) == 7, (
